...
#' In this example, we define a variable x with the value 5. We use an if-else
#' statement to check if x is greater than 3. If the condition x > 3 is TRUE,
#' the message "x is greater than 3" is printed.
#'
...
#' if-else-if ladder to check if y is less than 5, between 5 and 10, or greater
#' than 10. Depending on the value of y, the corresponding message is printed.
#'
...
#' In this example, we define a variable z with the value 7. We use the ternary
#' operator to assign a value to the variable result based on the condition z >
#' 5. If the condition is TRUE, the value "z is greater than 5" is assigned to
#' result. If the condition is FALSE, the value "z is not greater than 5" is
#' assigned to result.
#'
#' ## Loops (for, while)
#'
...
#' In this example, we use a for loop to iterate over a sequence of numbers from
#' 1 to 5. For each value of i in the sequence, the value of i is printed.
#'
#' ### While Loops
#'
...
#' In this example, we define a variable j with the value 1. We use a while loop
#' to repeat a block of code until j is greater than 5. For each iteration of
#' the loop, the value of j is printed, and j is incremented by 1. In Python,
#' shorthand operators are used to perform an arithmetic operation and update a
#' variable in a single step. Instead we need to use the following syntax in R
#' to iterate a variable and update it:
#'
#' x <- 5 x <- x + 1
#'
...
#' conditional statements (if-else), for loops, and while loops. These control
#' structures are essential for controlling the flow of execution in a program
#' and performing different actions based on conditions. By using conditional
#' statements and loops, we can make our code more flexible and powerful,
#' allowing us to solve a wide range of problems in R.
#'
